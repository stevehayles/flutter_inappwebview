...
typedef PermissionRequestedDelegate = FutureOr<WebviewPermissionDecision> Function(
...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  final StreamController<bool> _containsFullScreenElementChangedStreamController =
  ...
  Future<void> addVirtualHostNameMapping(
      String hostName, String folderPath, WebviewHostResourceAccessKind accessKind) async {
    if (_isDisposed) {
      return;
    }

    return _methodChannel
        .invokeMethod('setVirtualHostNameMapping', [hostName, folderPath, accessKind.index]);
  }
  ...
  Future<void> _setPointerUpdate(WebviewPointerEventKind kind, int pointer, Offset position,
    ...
    return _methodChannel.invokeMethod(
        'setPointerUpdate', [pointer, kind.index, position.dx, position.dy, size, pressure]);
  }
  ...
  Future<void> _setPointerButtonState(PointerButton button, bool isDown) async {
    ...
    assert(value.isInitialized);
    return _methodChannel.invokeMethod(
        'setPointerButton', <String, dynamic>{'button': button.index, 'isDown': isDown});
  }
                ...
                ? Listener(
                    ...
                    onPointerDown: (ev) {
                      ...
                      if (ev.kind == PointerDeviceKind.touch) {
                        _controller._setPointerUpdate(WebviewPointerEventKind.down, ev.pointer,
                    ...
                    onPointerUp: (ev) {
                      _pointerKind = ev.kind;
                      if (ev.kind == PointerDeviceKind.touch) {
                        _controller._setPointerUpdate(WebviewPointerEventKind.up, ev.pointer,
                            ev.localPosition, ev.size, ev.pressure);
                        return;
                      }
                    ...
                    onPointerMove: (ev) {
                      _pointerKind = ev.kind;
                      if (ev.kind == PointerDeviceKind.touch) {
                        _controller._setPointerUpdate(WebviewPointerEventKind.update, ev.pointer,
                            ev.localPosition, ev.size, ev.pressure);
                      } else {
                        _controller._setCursorPos(ev.localPosition);
                      }
                    ...
                    child: MouseRegion(
                        cursor: _cursor, child: Texture(textureId: _controller._textureId)),
                  )
                : const SizedBox()));
  }
